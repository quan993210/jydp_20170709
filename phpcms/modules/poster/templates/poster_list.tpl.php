...
?>   
	...
	<td align="center"><?php if($info['status'] == 0) { echo "待通过"; } else { echo "已审核"; }?></td>
	<td align="center">
		<?php if($info['status'] == 0 && $users['roleid'] == 1) { ?>
			<a  href="index.php?m=poster&c=poster&a=poster_check&id=<?php echo $info['id'];?>&pc_hash=<?php echo $_SESSION['pc_hash'];?>&menuid=<?php echo $_GET['menuid']?>" onClick="return confirm('确定审核?');">审核</a>|
		<?php } ?>
		<a href="index.php?m=poster&c=poster&a=edit&id=<?php echo $info['id'];?>&pc_hash=<?php echo $_SESSION['pc_hash'];?>" ><?php echo L('edit')?></a>|
