...
class poster extends admin {
	...
	public function init() {
		...
		$users = $admin_db->get_one(array('userid'=>$userid));
	...
	public function add() {
		if (isset($_POST['dosubmit'])) {
			...
			$admin_db = pc_base::load_model('admin_model');
			$userid = $_SESSION['userid'];
			$infos = $admin_db->get_one(array('userid'=>$userid));
			//非超级管理员的后台操作做添加广告需要审核 0待审核1审核通过
			if($infos['roleid'] ==1){
				$poster['status'] = 1;
			}else{
				$poster['status'] = 0;
	...
	public function poster_check() {
		...
		//非超级管理员的后台操作做添加广告需要审核 0待审核1审核通过
		if($infos['roleid'] !=1){
			showmessage('非管理员不能审核广告', HTTP_REFERER);
		}

		$this->db->update(array('status'=>1), array('id'=>$_GET['id']));
		showmessage('审核通过！');
